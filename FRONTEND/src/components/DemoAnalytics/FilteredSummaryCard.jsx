...
const FilteredSummaryCards = () => {
  ...
  const totalURLs = useSelector(
    (state) => state.demoStats?.data?.filteredSummary?.total_urls
  );
  const totalClicks = useSelector(
    (state) => state.demoStats?.data?.filteredSummary?.total_clicks
  );
  const avgClicks = useSelector(
    (state) => state.demoStats?.data?.filteredSummary?.avg_clicks_per_url
  );

  // (Optional) grab the active filter range if you have it in state
  const { startDate, endDate } = useSelector(
    (state) => state.demoStats?.filters ?? {}
  );
